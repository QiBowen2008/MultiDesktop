...
!\\\[](https://github.com/QiBowen2008/MultiDesktop/blob/net10rebuild/pics/1.png?raw=true)
...
1\. \*\*优先方案\*\*：通过 `SHSetKnownFolderPath(FOLDERID\\\_Desktop, ...)` 修改桌面文件夹路径，无需重启 explorer。
...
通过 `SystemParametersInfo(SPI\\\_SETDESKWALLPAPER, ...)` 设置壁纸，同步写入注册表：
